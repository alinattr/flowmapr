...
import { Copy, Check, RotateCcw, FileDown, Pencil } from 'lucide-react'
import { useTheme } from '@/lib/theme/ThemeProvider'
import { AppNavbar } from '@/components/shared/AppNavbar'
import { AppSidebar } from '@/components/shared/AppSidebar'
import { createClient } from '@/lib/supabase/client'

interface ExplainDiagramShellProps {
  email: string
...
const cleanSection = (text: string) =>
  text.replace(/(?:\s+[A-Z]{2})+$/, '').trim()


export function ExplainDiagramShell({
  email,
  fullName,
  plan,
  generationsRemaining,
}: ExplainDiagramShellProps) {
  ...
  const [docTitle, setDocTitle] = useState('Diagram Explanation')
  ...
  // ── Auto-save to artifacts table (silent background save) ─────────────────
  const saveArtifact = async (explanation: string, fileName: string) => {
    try {
      ...
      console.warn('[explain-diagram] artifact save failed:', err)
    }
  }
  ...
  const handleExplain = async () => {
    ...
    try {
      ...
      setResult({ explanation: data.explanation, fileName, analyzedAt: new Date() })
      setDocTitle('Diagram Explanation')
      setEditingTitle(false)

      // Save artifact silently in background
      saveArtifact(data.explanation, fileName)
    } catch {
  ...
  const parseSections = (text: string) => {
    ...
    for (const title of titles) {
      ...
      const body = cleanSection(
        nextIdx === Infinity ? afterTitle : afterTitle.slice(0, nextIdx - idx - title.length)
      )
      sections.push({ title, body })
    }
    if (sections.length === 0) sections.push({ title: 'EXPLANATION', body: cleanSection(text) })
    return sections
  }
    ...
    try {
      ...
      container.innerHTML = `
        <div style="border-bottom:2px solid #6366F1;padding-bottom:20px;margin-bottom:36px;">
          ...
          <div style="font-size:22px;font-weight:700;color:#111827;margin-bottom:6px;">${escapeHtml(docTitle)}</div>
          ${isFreePlan ? `<div style="font-size:12px;color:#6B7280;font-family:Inter,sans-serif;">Generated by Flowmapr · ${dateStr}</div>` : ''}
        </div>
        ${sections.map(s => `
        ...
        `).join('')}
        ${isFreePlan ? '<div style="margin-top:56px;padding-top:16px;border-top:1px solid #E5E7EB;font-family:Inter,sans-serif;font-size:11px;color:#9CA3AF;text-align:center;">flowmapr.com</div>' : ''}
      `
      ...
      const safeName = docTitle.replace(/\.[^.]+$/, '').replace(/[^a-z0-9]/gi, '-')
  ...
  const sections = result ? parseSections(result.explanation) : []
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div style={{
            ...
            overflowY: 'auto', padding: '28px 24px', gap: 0,
          }}>
            {/* Header */}
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {!file ? (
                <div
                  ...
                  style={{
                    ...
                    transition: 'all 0.15s ease', padding: '24px 16px',
                  }}
                >
                  ...
                  <div style={{ fontSize: 12, color: T.dropzoneText, fontFamily: 'Inter, sans-serif' }}>or click to browse</div>
                  <div style={{ fontSize: 11, color: T.dropzoneSubtext, fontFamily: 'Inter, sans-serif', marginTop: 4, textAlign: 'center' }}>
                    PNG, JPG, JPEG, WEBP · Max 10MB
                  </div>
                </div>
              ) : (
                <div style={{ borderRadius: 12, border: `1px solid ${T.previewBorder}`, background: T.previewBg, overflow: 'hidden' }}>
                  {preview && (
                  ...
                  <div style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, color: T.fileNameColor, fontFamily: 'Inter, sans-serif', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.name}</div>
                      <div style={{ fontSize: 11, color: T.fileSizeColor, fontFamily: 'Inter, sans-serif', marginTop: 1 }}>{formatBytes(file.size)}</div>
                    </div>
                    <button onClick={handleRemove} style={{ background: 'none', border: 'none', color: T.fileSizeColor, cursor: 'pointer', fontSize: 13, padding: 4, flexShrink: 0 }}>
                      ✕ Remove
                    </button>
                  </div>
                </div>
              ...
              <input ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/jpg,image/webp" style={{ display: 'none' }} onChange={onFileChange} />
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderRadius: 8, overflow: 'hidden', border: `1px solid ${T.toggleBorder}`, background: T.toggleBg }}>
                {(['brief', 'detailed'] as const).map(level => (
                  <button key={level} onClick={() => setDetailLevel(level)} style={{
          ...
          {/* ── RIGHT PANEL ── */}
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: T.rightBg, overflowY: 'auto', padding: 28 }}>
            ...
            {/* Result card */}
            {result && (
              <div style={{ borderRadius: 12, background: T.cardBg, border: `1px solid ${T.cardBorder}`, overflow: 'hidden', maxWidth: 760, width: '100%' }}>
                ...
                <div style={{ padding: '16px 20px', background: T.cardHeaderBg, borderBottom: `1px solid ${T.cardHeaderBorder}` }}>
                  {editingTitle ? (
                    <input
                      ...
                      onBlur={() => {
                        setDocTitle((prev) => prev.trim() || 'Diagram Explanation')
                        setEditingTitle(false)
                      }}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                ...
                <div style={{ padding: '14px 20px', borderTop: `1px solid ${T.cardHeaderBorder}`, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  <button onClick={handleExportPDF} disabled={isExporting} style={{
                    ...
                    background: T.actionBg, color: T.actionColor,
                    fontSize: 12, fontWeight: 500, fontFamily: 'Inter, sans-serif',
                  ...
                  {isFreePlan && (
                    <div style={{ width: '100%', fontSize: 11, color: T.cardMetaColor, fontFamily: 'Inter, sans-serif' }}>
                      🔒 PDF includes "Generated by Flowmapr" watermark.{' '}
                      <a href="/settings#billing" style={{ color: '#6366F1', textDecoration: 'underline' }}>
                        Upgrade to Basic to remove it
                      </a>
                    </div>
                  )}

                  {/* Explain another */}
                  <button onClick={handleExplainAnother} style={{
                    flex: 1, minWidth: 130, padding: '9px 14px', borderRadius: 8,
                    border: '1px solid rgba(99,102,241,0.25)',
